...
class Login(Resource):
    def post(self):
        ...
        if user and user.authenticate(password):
            ...
            # session['user_id'] = user.id
            # with session, we are storing the session in the backend and a cookie in the frontend
            # with JWT, we are just storing the token on the frontend
...
# Logout no longer needed, backend is no longer controlling if the user is logged in or not
# JWT token will be cleared on the frontend
